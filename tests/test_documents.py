...
async def _make_document(
    ...
    doc = Document(
        name=name, project_id=project.id, owner_id=owner.id, url=url, is_pending=False
    )
    db.add(doc)
    await db.flush()
    return doc
...
async def test_upload_multiple_files(
    ...
    response = await client.post(
        f"/project/{project.id}/documents", files=files, headers=owner_headers
    )
    assert response.status_code in [200, 201], "upload should succeed"
    assert mock_s3["upload"].call_count == 2, "each uploaded file should be sent to S3"
...
async def test_list_documents_project_not_found(
    ...
    response = await client.get("/project/999999/documents", headers=owner_headers)
...
async def test_download_document_owner(
    ...
    doc = await _make_document(
        db_session, project, test_owner, "spec.pdf", "projects/1/documents/spec.pdf"
    )
    ...
    assert "download_url" in data, "download response should include download_url"
...
async def test_update_document_owner(
    ...
    doc = await _make_document(
        db_session, project, test_owner, "old.pdf", "projects/1/documents/old.pdf"
    )
    ...
    (
        mock_s3["delete"].assert_called_once_with("projects/1/documents/old.pdf"),
        "old file should be deleted from S3",
    )
    mock_s3["upload"].assert_called_once(), ("new file should be uploaded to S3")
...
async def test_update_document_refreshes_db_record(
    ...
    doc = await _make_document(
        db_session, project, test_owner, "stale.pdf", "projects/1/documents/stale.pdf"
    )
    old_url = doc.url
...
async def test_update_document_not_found(client: AsyncClient, owner_headers: dict):
    response = await client.put(
        "/document/999999",
...
async def test_delete_document_owner_success(
    ...
    doc = await _make_document(
        db_session, project, test_owner, "trash.pdf", "projects/1/documents/trash.pdf"
    )
    ...
    (
        mock_s3["delete"].assert_called_once_with("projects/1/documents/trash.pdf"),
...
async def test_delete_document_participant_blocked(
    ...
    response = await client.delete(f"/document/{doc.id}", headers=participant_headers)
...
async def test_delete_document_not_found(client: AsyncClient, owner_headers: dict):
