...
async def test_project_crud_owner_flow(
    ...
    """Verify that an Owner can completely manage (create, read, update) projects (ProjectOut compliance)"""
    # Create Project (ProjectCreate schema requires name length >= 4)
    create_payload = {"name": "Test Dashboard",
                      "description": "High level dashboard"}
    response = await client.post("/projects", json=create_payload, headers=owner_headers)
    assert response.status_code in [
        200, 201], "Project creation should succeed with 200 or 201"

    project_data = response.json()
    project_id = project_data["id"]
    assert project_data["name"] == "Test Dashboard", "Project name should match the creation payload"
    ...
    assert response.status_code == 200, "Project info retrieval should succeed with 200"
    assert response.json()[
        "name"] == "Test Dashboard", "Project name should match the creation payload"

    # Update details
    update_payload = {"name": "Updated Dashboard",
                      "description": "Updated Desc"}
    response = await client.put(f"/project/{project_id}/info", json=update_payload, headers=owner_headers)
    assert response.status_code == 200, "Project update should succeed with 200"
    assert response.json()[
        "name"] == "Updated Dashboard", "Project name should match the update payload"
...
async def test_project_validation_rules(client: AsyncClient, owner_headers: dict):
    ...
    assert response.status_code == 422, "Project creation should fail with 422 for invalid name"
...
async def test_project_sharing_and_participant_restrictions(
    ...
    """Verify that invited participants can modify, but are blocked from deleting projects"""
    project = Project(name="Shared Project",
                      description="Shared", owner_id=test_owner.id)
    ...
    assert response.status_code in [
        200, 201], " Inviting participant should succeed with 200 or 201"

    # Verify participant role mapping in DB
    stmt = select(Access).where(Access.project_id == project.id,
    ...
    assert access.role == "participant", " Participant role should be correctly assigned in the Access table"

    # Participant reads project info (Succeeds)
    response = await client.get(f"/project/{project.id}/info", headers=participant_headers)
    assert response.status_code == 200, "Participant should be able to read project info with 200"

    # Participant deletes project (Blocked with 403)
    response = await client.delete(f"/project/{project.id}", headers=participant_headers)
    assert response.status_code == 403, "Participant should be blocked from deleting the project with 403"
...
async def test_unauthorized_user_blocked_completely(
    ...
    """Verify that a user without access to a project is completely blocked from reading or writing project info (403 or 404)"""
    project = Project(name="Secret Project",
                      description="Hidden", owner_id=test_owner.id)
    db_session.add(project)
    await db_session.flush()
    ...
    assert response.status_code in [
        403, 404], "Unrelated user should be blocked from reading project info"

    # Unrelated user attempts write
    update_payload = {"name": "Hacked!", "description": "Hacked!"}
    response = await client.put(f"/project/{project.id}/info", json=update_payload, headers=other_headers)
    assert response.status_code in [
        403, 404], "Unrelated user should be blocked from writing project info"
...
async def test_invite_permission_restricted_to_owner(
    ...
    """Verify that ONLY the project owner can invite users to a project, and participants cannot invite others (403)"""
    # Create and flush project first to generate project.id
    project = Project(name="Core Project",
                      description="Core desc", owner_id=test_owner.id)
    db_session.add(project)
    await db_session.flush()

    # Assign the participant to the project using the generated ID
    access = Access(user_id=test_participant.id,
                    project_id=project.id, role="participant")
    ...
    assert response.status_code == 403, "Participant should be blocked from inviting users to the project with 403"
