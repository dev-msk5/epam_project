...
@router.post("/projects", response_model=Project, response_code=201)  # Created
...
@router.get("/projects", response_model=list[Project], response_code=200)  # OK
...
@router.get("/project/{project_id}/info", response_code=200)
async def get_project_info(project_id: int):
    return {"message": f"Project info for project {project_id}"}


@router.put("/project/{project_id}/info", response_code=200)
...
@router.delete("/project/{project_id}", response_code=204)  # No Content
async def delete_project(project_id: int):
    # only by the projects owner
    return {"message": f"Project {project_id} deleted"}


@router.post("/project/{project_id}/invite?user={login}", response_code=200)
...
# Optional
@router.get("/project/{project_id}/share?with={email}", response_code=200)
